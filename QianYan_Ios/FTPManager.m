...
- (BOOL) _downloadFile:(NSString*)fileName toDirectory:(NSURL*)directoryURL fromServer:(FMServer*)server {
    ...
    NSURL *url = [[server.destination ftpURLForPort:server.port] URLByAppendingPathComponent:fileName] ;
    NSLog(@"url = %@",url) ;
    CFReadStreamRef readStream = CFReadStreamCreateWithFTPURL(NULL, (__bridge CFURLRef)url);
    And(success, (readStream != NULL));
    Check(success);
    self.serverReadStream = (__bridge_transfer NSInputStream*) readStream;
