...
@interface KxMovieViewController () <UITabBarDelegate> {
    ...
    UIView              *_topHUD ;
    UIToolbar           *_topBar ;
    UITabBar            *_bottomBar ;      //TabBar
    UIView              *_bottomBarMaskView ;

    UITabBarItem        *_playBtn ;
    UITabBarItem        *_pauseBtn ;
    
    UITabBarItem        *_screenLockBtn ;
    UITabBarItem        *_screenShotBtn ;
    UITabBarItem        *_talkBtn ;
    UITabBarItem        *_shortVideoBtn ;
    ...
    UIButton            *_addBtn ;//保存到相册
    UIButton            *_shareBtn ;
    UIImageView         *_thumbnailImageView ;
    UIImageView         *_liveImageView ;
    UIImageView         *_timeAxisImageView ;
    UILabel             *_timeLabel ;
    UILabel             *_titleLabel ;
...
- (void)loadView {
    ...
    _settingsBtnItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"视频-设置.png"] style:UIBarButtonItemStylePlain target:self action:@selector(settingsBtnClicked:)] ;
    self.navigationItem.rightBarButtonItem = _settingsBtnItem ;
    ...
    _activityIndicatorView.center = CGPointMake(160, 220) ;
...
_messageLabel.hidden = YES ;
    ...
    _bottomBar = [[UITabBar alloc] initWithFrame:CGRectMake(0, height-botH, width, botH)] ;
    _bottomBar.tintColor = [UIColor blackColor] ;
    _bottomBar.delegate = self ;
    _bottomBar.selectedImageTintColor = [UIColor colorWithRed:37.0/255 green:186.0/255 blue:244.0/255 alpha:1] ;
    
    _bottomBarMaskView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 568, 50)] ;
    _bottomBarMaskView.backgroundColor = [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.8] ;
    ...
    _settingsButton = [[UIButton alloc] initWithFrame:CGRectMake(530, 20, 20, 20)] ;
    [_settingsButton setImage:[UIImage imageNamed:@"视频-设置.png"] forState:UIControlStateNormal] ;
    [_settingsButton addTarget:self action:@selector(settingsBtnClicked:) forControlEvents:UIControlEventTouchUpInside] ;
    
    _topHUD    = [[UIView alloc] initWithFrame:CGRectMake(0,0,568,topH)] ;
    _topHUD.backgroundColor = [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.8] ;
    _topBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 0, 568, topH)] ;
    _topBar.backgroundColor = [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.8] ;
    

    _infoButton = [[UIButton alloc] init] ;
    [_infoButton setImage:[UIImage imageNamed:@"设置.png"] forState:UIControlStateNormal] ;
    ...
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(253, 20, 60, 20)] ;
    _titleLabel.textColor = [UIColor whiteColor] ;
    _titleLabel.text = @"小酒馆" ;
    
    [_topHUD addSubview:_doneButton] ;
    [_topHUD addSubview:_settingsButton] ;
    [_topHUD addSubview:_titleLabel] ;
    
    // middle hud
    _liveImageView = [[UIImageView alloc] initWithFrame:CGRectMake(20, 71, 50, 22)] ;
    _liveImageView.image = [UIImage imageNamed:@"视频-live直播.png"] ;
    ...
    _timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(90, 71, 180, 22)] ;
    _timeLabel.text = date ;
    [_timeLabel setTextColor:[UIColor grayColor]] ;
    [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(UpdateTimeLabel:) userInfo:nil repeats:NO] ;
    
    _addBtn = [[UIButton alloc] initWithFrame:CGRectMake(143, 415, 110, 35)] ;
    [_addBtn setImage:[UIImage imageNamed:@"截图-保存到相册.png"] forState:UIControlStateNormal] ;
    [_addBtn addTarget:self action:@selector(saveScreenShot2AlbumBtnClicked:) forControlEvents:UIControlEventTouchUpInside] ;
    
    _shareBtn = [[UIButton alloc] initWithFrame:CGRectMake(143, 460, 110, 35)] ;
    [_shareBtn setImage:[UIImage imageNamed:@"截图-分享到社交圈.png"] forState:UIControlStateNormal] ;

    _thumbnailImageView = [[UIImageView alloc] initWithFrame:CGRectMake(22, 410, 90, 90)] ;
    _thumbnailImageView.image = [UIImage imageNamed:@"截图-图片.png"] ;
    
    _fullScreenBtn = [[UIButton alloc] initWithFrame:CGRectMake(274, 340, 46, 46)] ;
    [_fullScreenBtn setImage:[UIImage imageNamed:@"视频播放-全屏.png"] forState:UIControlStateNormal] ;
    [_fullScreenBtn addTarget:self action:@selector(fullScreenBtnClicked:) forControlEvents:UIControlEventTouchUpInside] ;
    
    _timeAxisImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 340, 274, 46)] ;
    _timeAxisImageView.image = [UIImage imageNamed:@"视频-时间轴.png"] ;
    ...
    [self.view addSubview:_timeAxisImageView] ;

    [self setScreenShotHidden:YES] ;
    
    // bottom hud

    _playBtn = [[UITabBarItem alloc] initWithTitle:@"播放" image:[UIImage imageNamed:@"播放.png"] tag:0] ;
    
    _pauseBtn = [[UITabBarItem alloc] initWithTitle:@"停止" image:[UIImage imageNamed:@"停止.png"] tag:0] ;

    _screenLockBtn = [[UITabBarItem alloc] initWithTitle:@"锁屏" image:[UIImage imageNamed:@"锁屏.png"] tag:1] ;

    _screenShotBtn = [[UITabBarItem alloc] initWithTitle:@"截图" image:[UIImage imageNamed:@"截图.png"] tag:2] ;

    _talkBtn = [[UITabBarItem alloc] initWithTitle:@"语音" image:[UIImage imageNamed:@"语音.png"] tag:3] ;

    _shortVideoBtn = [[UITabBarItem alloc] initWithTitle:@"短视频" image:[UIImage imageNamed:@"短视频.png"] tag:4] ;
...
- (void)viewDidLoad {
    [super viewDidLoad] ;
    //    UIBarButtonItem *item = [[UIBarButtonItem alloc] initWithTitle:@"相机" style:UIBarButtonItemStylePlain target:self action:@selector(backBtnClicked:)] ;
...
- (void)doneDidTouch:(id)sender {
    ...
    [_liveImageView setHidden:NO] ;
    [_timeAxisImageView setHidden:NO] ;
    [_timeLabel setHidden:NO] ;
    [_fullScreenBtn setHidden:NO] ;
    ...
    _bottomBar.selectedImageTintColor = [UIColor colorWithRed:37.0/255 green:186.0/255 blue:244.0/255 alpha:1] ;
    _bottomBar.frame = CGRectMake(0, 518, 320, 50) ;
    [_bottomBarMaskView removeFromSuperview] ;
    [self.view addSubview:_bottomBar] ;

    _glView.frame = CGRectMake(-84, 184, 240, 320) ;
...
- (void)fullScreenBtnClicked:(id)sender {
//    NSMutableDictionary *parameters = [NSMutableDictionary dictionary] ;
//    LandScapeKxMovieViewController *lskvc = [LandScapeKxMovieViewController movieViewControllerWithContentPath:@"http://www.qeebu.com/newe/Public/Attachment/99/52958fdb45565.mp4" parameters:parameters] ;
//    
//    [self presentViewController:lskvc animated:YES completion:nil] ;
    if (self.playing) {
        [self.navigationController setNavigationBarHidden:YES] ;
        [_liveImageView setHidden:YES] ;
        [_timeAxisImageView setHidden:YES] ;
        ...
        [_glView addSubview:_topBar] ;
        [_glView addSubview:_topHUD] ;
        
        [_bottomBar removeFromSuperview] ;
        _bottomBar.frame = CGRectMake(0, 270, 568, 50) ;
        _bottomBar.opaque = NO ;
        _bottomBar.selectedImageTintColor = [UIColor colorWithRed:249.0/255 green:171.0/255 blue:24.0/255 alpha:0.8] ;
        [_glView addSubview:_bottomBar] ;
        [_bottomBar insertSubview:_bottomBarMaskView atIndex:1] ;
        
        _glView.frame = CGRectMake(0, 0, 568, 320) ;
        float centerX = _glView.bounds.size.width/2 ;
        float centerY = _glView.bounds.size.height/2 ;
        float x = 160 ;
        float y = 160 ;
        x = x - centerX ;
        y = y - centerY ;
...
- (void)tabBar:(UITabBar *)tabBar didSelectItem:(UITabBarItem *)item {
    if ([item.title isEqualToString:@"停止"]) {
        [self pause] ;
        [self updateBottomBar] ;
        [_fullScreenBtn setEnabled:YES] ;
        [self setScreenShotHidden:YES] ;
    }else if([item.title isEqualToString:@"播放"]) {
        [self play] ;
        [self updateBottomBar] ;
        [_fullScreenBtn setEnabled:YES] ;
        [self setScreenShotHidden:YES] ;
    }else if([item.title isEqualToString:@"锁屏"]) {
        [_fullScreenBtn setEnabled:NO] ;
        [self setScreenShotHidden:YES] ;
    }else if([item.title isEqualToString:@"截图"]) {  // 截图
        [_fullScreenBtn setEnabled:YES] ;
        [self setScreenShotHidden:NO] ;
    }else if([item.title isEqualToString:@"语音"]) {
        [_fullScreenBtn setEnabled:YES] ;
        [self setScreenShotHidden:YES] ;
    }else if([item.title isEqualToString:@"短视频"]) {
        [_fullScreenBtn setEnabled:YES] ;
        [self setScreenShotHidden:YES] ;
    }
...
- (void)setScreenShotHidden:(BOOL)hidden {
    [_addBtn setHidden:hidden] ;
...
#pragma mark - 视频位置
    if (_decoder.validVideo) {
        _glView = [[KxMovieGLView alloc] initWithFrame:CGRectMake(0, 100, 320, 240) decoder:_decoder] ;
...
#pragma mark - 刷新底部item

- (void)updateBottomBar
{
    UITabBarItem *playPauseBtn = self.playing ? _pauseBtn : _playBtn ;
    [_bottomBar setItems:@[playPauseBtn, _screenLockBtn, _screenShotBtn, _talkBtn, _shortVideoBtn] animated:NO] ;
}
